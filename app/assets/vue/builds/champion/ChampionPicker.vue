...
import type { ChampionOption } from '../catalog/catalogTypes'
import type { ChampionLabels, UiLabels } from '../editor/editorLabels'
...
// Haystacks are precomputed so typing only re-runs the includes(). The id joins
// the name so both "Kai'Sa" and "Kaisa" find the champion.
const searchIndex = computed(() =>
    (props.options ?? []).map((champion) => ({
        champion,
        searchText: normalizeSearchText(`${champion.name} ${champion.id}`),
    })),
)
...
const selected = computed(
    () => (props.options ?? []).find((c) => c.id === props.selectedId) ?? null,
)
            ...
            <span class="forge-champ-toggle__id">
                ...
                <span class="forge-champ-toggle__label">
                    <span class="forge-hint">{{
                        selectedId ? labels.selected : labels.title
                    }}</span>
                    <span
                        v-if="selectedId"
                        class="font-beaufort text-base uppercase tracking-wide text-gold-bright"
                    >
