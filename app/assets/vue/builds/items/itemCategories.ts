...
export const ITEM_CATEGORY_ALL = 'all' as const
...
const CATEGORY_TAGS: Record<Exclude<ItemCategoryKey, typeof ITEM_CATEGORY_ALL>, readonly string[]>
    = {
    attack: ['Damage', 'AttackSpeed', 'CriticalStrike', 'ArmorPenetration', 'LifeSteal', 'OnHit'],
    magic: [
        'SpellDamage', 'MagicPenetration', 'Mana', 'ManaRegen', 'SpellVamp', 'CooldownReduction',
        'AbilityHaste',
    ],
    defense: ['Health', 'HealthRegen', 'Armor', 'SpellBlock', 'Tenacity'],
    mobility: ['Boots', 'NonbootsMovement'],
    utility: [
        'Consumable', 'Trinket', 'Vision', 'GoldPer', 'Jungle', 'Lane', 'Active', 'Aura', 'Slow',
        'Stealth',
    ],
}
...
export function matchesCategory(item: ItemOption, category: ItemCategoryKey): boolean {
    if (category === ITEM_CATEGORY_ALL) return true
