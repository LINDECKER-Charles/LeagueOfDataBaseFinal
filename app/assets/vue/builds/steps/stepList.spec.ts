...
describe('item bounds', () => {
    ...
    it('removes and reorders items within a step', () => {
        ...
        expect(moveItem(list, { step: 0, index: 2 }, -1)[0]?.items).toEqual(['a', 'c', 'b'])
        expect(moveItem(list, { step: 0, index: 0 }, -1)).toBe(list)
        expect(moveItem(list, { step: 1, index: 0 }, 1)).toBe(list) // unknown step
    })
})
...
describe('drop semantics (insertion indexes)', () => {
    ...
    it('reorders items inside a step to an insertion point', () => {
        const list = steps(['a', 'b', 'c'])
        expect(moveItemToIndex(list, { step: 0, index: 0 }, 3)[0]?.items).toEqual(['b', 'c', 'a'])
        expect(moveItemToIndex(list, { step: 0, index: 2 }, 0)[0]?.items).toEqual(['c', 'a', 'b'])
        expect(moveItemToIndex(list, { step: 0, index: 1 }, 99)[0]?.items).toEqual(['a', 'c', 'b'])
        expect(moveItemToIndex(list, { step: 0, index: 1 }, 1)).toBe(list)
        expect(moveItemToIndex(list, { step: 5, index: 0 }, 1)).toBe(list)
    })

    it('inserts an item at an exact clamped position, capacity-checked', () => {
        const list = steps(['a', 'c'])
        expect(insertItem(list, { step: 0, index: 1 }, 'b')[0]?.items).toEqual(['a', 'b', 'c'])
        expect(insertItem(list, { step: 0, index: -5 }, 'z')[0]?.items).toEqual(['z', 'a', 'c'])
        expect(insertItem(list, { step: 0, index: 99 }, 'z')[0]?.items).toEqual(['a', 'c', 'z'])

        const head = { step: 0, index: 0 }
        let full = steps([])
        for (let i = 0; i < MAX_ITEMS_PER_STEP; i++) full = insertItem(full, head, 'x')
        expect(insertItem(full, head, 'x')).toBe(full)
    })
    ...
    it('same-step transfers degrade to a reorder', () => {
        const list = steps(['a', 'b', 'c'])
        expect(transferItem(list, { step: 0, index: 0 }, { step: 0, index: 3 })[0]?.items)
            .toEqual(['b', 'c', 'a'])
    })
...
describe('restingIndex (position announced to screen readers)', () => {
    it('matches where the entry actually lands, including out-of-range drops', () => {
        const list = steps(['a', 'b', 'c'])
        const cases: Array<[from: number, insert: number]> = [
            [0, 3], [2, 0], [1, 99], [1, -4], [0, 0], [2, 3],
        ]
        for (const [from, insert] of cases) {
            const moved = moveItemToIndex(list, { step: 0, index: from }, insert)
            const dragged = list[0]?.items[from]
            expect(moved[0]?.items[restingIndex(from, insert, 3)]).toBe(dragged)
        }
    })

    it('matches where a dragged step lands', () => {
        const list = steps(['a'], ['b'], ['c'])
        const moved = moveStepToIndex(list, 0, 3)
        expect(moved[restingIndex(0, 3, list.length)]?.items[0]).toBe('a')
    })
})
...
describe('gold totals', () => {
    ...
    it('sums a step, counting unknown ids (ghosts) as zero', () => {
        expect(stepGold({ label: '', note: null, items: ['1055', '3006', 'gone'] }, goldOf))
            .toBe(1550)
    })
