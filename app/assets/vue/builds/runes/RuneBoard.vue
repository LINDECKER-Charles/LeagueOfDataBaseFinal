...
import CatalogState from '../catalog/CatalogState.vue'
import type { RunePerk, RuneTree } from '../catalog/catalogTypes'
import { GHOST_SLOT, KEYSTONE_SLOT, SECONDARY_PICKS, type RuneDraft } from './runeRules'
import type { RunesLabels, UiLabels } from '../editor/editorLabels'
...
const primaryTree = computed(
    () => (props.trees ?? []).find((t) => t.id === props.draft.primaryStyleId) ?? null
)
const secondaryTree = computed(
    () => (props.trees ?? []).find((t) => t.id === props.draft.secondaryStyleId) ?? null
)
const secondaryChoices = computed(
    () => (props.trees ?? []).filter((t) => t.id !== props.draft.primaryStyleId)
)
...
function slotName(slotIndex: number): string {
    return slotIndex === KEYSTONE_SLOT
        ? props.labels.keystone
        : formatTemplate(props.labels.slot, { n: slotIndex })
}
...
const secondaryGhosts = computed(
    () => props.draft.secondaryPicks.filter((p) => p.slotIndex === GHOST_SLOT)
)
</script>
                ...
                <div :class="pathClass(primaryTree)">
                    ...
                    <div class="forge-trees">
                        <button
                            ...
                            :class="[
                                pathClass(tree),
                                { 'forge-tree--on': tree.id === draft.primaryStyleId }
                            ]"
                    ...
                    <div v-if="primaryTree" class="mt-4">
                        <div
                            ...
                            class="forge-slot"
                            :class="{
                                'forge-slot--picked': draft.primaryPerks[slotIndex] !== null,
                            }"
                        >
                            ...
                            <button
                                ...
                                class="forge-perk"
                                :class="{
                                    'forge-perk--big': slotIndex === 0,
                                    'forge-perk--on': isPrimaryPick(slotIndex, perk.id)
                                }"
                                :aria-pressed="isPrimaryPick(slotIndex, perk.id)"
                                :title="perk.shortDesc
                                    ? `${perk.name} — ${perk.shortDesc}`
                                    : perk.name"
                            ...
                            <span
                                v-if="primaryGhost(slotIndex, perks) !== null"
                                class="hx-chip forge-ghost"
                                :title="ui.ghost"
                            >
                ...
                <div v-if="draft.primaryStyleId !== null" :class="pathClass(secondaryTree)">
                    ...
                    <div class="forge-trees">
                        <button
                            ...
                            :class="[
                                pathClass(tree),
                                { 'forge-tree--on': tree.id === draft.secondaryStyleId }
                            ]"
                    ...
                    <div v-if="secondaryGhosts.length" class="mt-3 flex flex-wrap gap-2">
                        <span
                            v-for="ghost in secondaryGhosts"
                            :key="ghost.perkId"
                            class="hx-chip forge-ghost"
                            :title="ui.ghost"
                        >
                    ...
                    <div v-if="secondaryTree" class="mt-4">
                        <template
                            v-for="(perks, slotIndex) in secondaryTree.slots"
                            :key="slotIndex"
                        >
                            <div
                                v-if="slotIndex > 0"
                                ...
                                :class="{
                                    ...
                                    'forge-slot--locked':
                                        isSecondaryFull && !isSecondarySlotUsed(slotIndex),
                                }"
                            >
                                <span class="forge-slot__name">{{ slotName(slotIndex) }}</span>
                                <button
