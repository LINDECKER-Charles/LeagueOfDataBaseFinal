...
const modal = ref<{ open: boolean; html: string; type: string }>({
    open: false,
    html: '',
    type: 'info'
})
...
/** Kept in step with the `duration-300` exit transition of the toast card. */
const EXIT_ANIMATION_MS = 220
/** First toast waits a frame or two so its entrance transition actually runs. */
const ENTER_DELAY_MS = 40
/** Cascade between two stacked toasts. */
const ENTER_STAGGER_MS = 70
const TOAST_LIFETIME_MS = 5000

const ACCENT: Record<string, string> = {
    success: 'border-emerald-400/50 text-emerald-200',
    error: 'border-danger/60 text-danger-light',
...
onMounted(() => {
    ...
    ;(props.messages ?? []).forEach((m, i) => {
        ...
        setTimeout(() => {
            const t = toasts.value.find((x) => x.id === id)
            if (t) t.visible = true
        }, ENTER_DELAY_MS + i * ENTER_STAGGER_MS)
        timers.set(id, setTimeout(() => dismiss(id), TOAST_LIFETIME_MS))
    })
})
...
<template>
    <div class="pointer-events-none fixed top-5 right-5 z-50 flex flex-col gap-3">
        <div
            v-for="t in toasts"
            :key="t.id"
            class="hextech-frame pointer-events-auto w-80 cursor-pointer px-4 py-3
        ...
        >
            <div class="flex items-start gap-3">
                <span class="mt-0.5 h-2 w-2 shrink-0 rounded-full bg-current
                             shadow-[0_0_8px_currentColor]" />
                <div
                    class="flex-1 truncate whitespace-pre-line break-words text-sm leading-5
                           text-text"
                    v-html="t.text"
                />
                <button
                    type="button"
                    class="-m-1 ml-1 inline-flex h-6 w-6 items-center justify-center rounded
    ...
    <Teleport to="body">
        <div v-if="modal.open" class="fixed inset-0 z-[60] grid place-items-center">
            <div class="absolute inset-0 bg-hextech-black/70 backdrop-blur-sm"
                 @click="closeModal" />
            <div
                ...
                aria-modal="true"
            >
                <div class="flex items-center justify-between gap-4 border-b border-gold-deep/40
                            px-5 py-4">
                    <h2 class="eyebrow">Message</h2>
                    <button
                        type="button"
                        class="rounded p-1 text-text-muted hover:text-gold-bright"
                        aria-label="Close"
                        @click="closeModal"
                    >
                ...
                <div
                    class="max-h-[68vh] overflow-auto px-5 py-4 text-[15px] leading-6
                           whitespace-pre-line break-words text-text"
                    v-html="modal.html"
                />
            </div>
        </div>
