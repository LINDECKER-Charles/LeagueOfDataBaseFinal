...
const scoreText = computed(
    () => (state.value.score > 0 ? `+${state.value.score}` : String(state.value.score)),
)
...
async function vote(direction: VoteDirection): Promise<void> {
    ...
    try {
        state.value = await postVote(direction)
    } catch {
...
/** The authoritative score, or a throw the optimistic move rolls back on. */
async function postVote(direction: VoteDirection): Promise<VoteState> {
...
<template>
    <div class="vote-box">
        ...
        <span class="vote-score" :class="scoreClass" :title="labels.score" aria-live="polite">{{
            scoreText
        }}</span>
