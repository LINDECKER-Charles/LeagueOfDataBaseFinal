...
const items = computed(() => [
    ...evaluatePasswordRules(password.value).map((rule) => ({
        ...
        isSatisfied: rule.satisfied,
    })),
    {
        id: 'match',
        label: props.labels.match,
        isSatisfied: passwordsMatch(password.value, confirmation.value),
    },
])
</script>
...
<template>
    <ul class="pwd-checklist" :class="{ 'pwd-checklist--touched': touched }" aria-live="polite">
        <li
            ...
            :class="{ 'pwd-checklist__item--ok': item.isSatisfied }"
        >
