...
const activeState = computed(
    () => (step.value === 'champion' ? catalog.champions : skinState.value)
)
...
const headTitle = computed(() =>
    step.value === 'champion'
        ? props.labels.chooseChampion
        : (champion.value?.name ?? props.labels.chooseSkin),
)
    ...
    <div ref="root">
        ...
        >
            ...
            <header class="picker-head">
                ...
                <button type="button" class="picker-close" :aria-label="labels.close"
                        @click="close">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                         stroke-width="2" aria-hidden="true">
            ...
            <div class="picker-list">
                <p v-if="activeState?.status === 'loading' || activeState?.status === 'idle'"
                   class="picker-note">
                    {{ labels.loading }}
                </p>
                <div v-else-if="activeState?.status === 'error'" class="picker-note">
                    <p>{{ labels.error }}</p>
                    <button type="button" class="picker-retry"
                            @click="retry">{{ labels.retry }}</button>
                </div>
                <template v-else>
                    <p v-if="visibleEntries.length === 0"
                       class="picker-note">{{ labels.noResults }}</p>
            ...
            <footer v-if="value !== ''" class="picker-foot">
                <button type="button" class="picker-remove"
                        @click="removeCurrent">{{ labels.remove }}</button>
            </footer>
        </dialog>
    </div>
</template>
