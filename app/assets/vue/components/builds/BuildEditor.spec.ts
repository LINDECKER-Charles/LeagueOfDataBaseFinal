...
import { clearPickerCatalogCache } from '../../builds/catalog/usePickerCatalog'
...
const GREAVES = {
    id: '3006', name: 'Berserker Greaves', image: null, gold: 1100, purchasable: true, tags: [],
}
const DORANS = {
    id: '1055', name: "Doran's Blade", image: null, gold: 450, purchasable: true, tags: [],
}
...
const labels = {
    ...
    context: {
        ...
        version: 'Patch',
        mode: 'Game mode',
        modeHint: 'Availability follows the mode.',
...
/** A saved build placing Greaves, so a context switch can ghost it. */
const initial = {
    championId: 'Aatrox',
    runes: {
        primaryStyleId: 0,
...
function mountEditor() {
    return mount(BuildEditor, { props: editorProps })
}
...
describe('BuildEditor island', () => {
    ...
    it('renders the game-context selects as real form fields', async () => {
        ...
        expect(wrapper.find('input[name="structure"]').attributes('value'))
            .toContain('"championId":"Aatrox"')
    })
    ...
    it('loads the three catalogs for the initial (version, mode) context', async () => {
        ...
        expect(urls.some((u) => u.includes(`/champions?version=${VERSION}&lang=${LANG}`)))
            .toBe(true)
        expect(urls.some((u) => u.includes(`/items?version=${VERSION}&lang=${LANG}&mode=sr`)))
            .toBe(true)
    })
    ...
    it('exposes the authoring language as a real form field that never reloads catalogs',
        async () => {
            const wrapper = mountEditor()
            await flushPromises()
            fetchMock.mockClear()
