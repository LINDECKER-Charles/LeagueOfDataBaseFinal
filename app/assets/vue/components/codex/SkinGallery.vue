...
const current = computed(() =>
    (openIndex.value === null ? null : props.skins[openIndex.value] ?? null))
...
<template>
    <div class="-mx-6 flex snap-x snap-proximity gap-4 overflow-x-auto px-6 pb-2">
        <figure
            ...
            :key="skin.num"
            class="w-72 shrink-0 snap-start scroll-ml-6"
        >
            <button
                type="button"
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <path d="M7.5 12 12 7.5 16.5 12 12 16.5z" />
                    </svg>
                    {{ skin.chromas.length }}
                </span>
            ...
            <figure class="relative z-10 w-[min(94vw,72rem)]" role="dialog" aria-modal="true">
                ...
                <figcaption class="mt-3 flex items-center justify-between gap-4 px-1">
                    <div class="min-w-0">
                        ...
                        <h2
                            class="truncate font-beaufort uppercase tracking-wide text-gold-bright"
                        >{{ current.name }}</h2>
                    </div>
...
.skin-tile {
    padding: 0;
    cursor: pointer;
    transition: transform 0.2s var(--ease-hextech, ease),
