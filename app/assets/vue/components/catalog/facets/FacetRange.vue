 ...
 * A selection equal to the full bounds is no selection at all.
 */
...
const emit = defineEmits<{ change: [range: RangeSelection | null] }>()
...
function commit(): void {
    ...
    emit('change', isWhole ? null : { min, max })
}
...
<template>
    <div class="facet facet--range" role="group" :aria-label="facet.label">
        <div class="facet__head">
            ...
            <span class="facet__readout" aria-live="polite">{{ format(low) }} – {{ format(high) }}</span>
            <button v-if="isActive" type="button" class="filter-clear"
                    @click="emit('change', null)">{{ labels.clear }}</button>
        </div>
        ...
        <div class="facet__fields">
            <label class="facet__field">
                ...
                <input type="number" :min="bounds.min" :max="bounds.max" :step="facet.step"
                       :value="low" @input="onLowInput" @change="commit">
            </label>
            <label class="facet__field">
                <span>{{ labels.max }}</span>
                <input type="number" :min="bounds.min" :max="bounds.max" :step="facet.step"
                       :value="high" @input="onHighInput" @change="commit">
            </label>
        </div>
    </div>
</template>
