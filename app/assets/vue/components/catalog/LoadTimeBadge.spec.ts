...
function fetchResponse(runtime: string | null): void {
    ...
    document.dispatchEvent(
        new CustomEvent('turbo:before-fetch-response', {
            detail: { fetchResponse: { response: { headers } } },
        }),
    )
}
...
function navEntry(dcl: number): void {
    vi.spyOn(performance, 'getEntriesByType').mockReturnValue(
        [
            { domContentLoadedEventEnd: dcl, responseEnd: dcl, startTime: 0 },
...
describe('LoadTimeBadge', () => {
    ...
    it('shows on a detail page (marker present) with server + client timings on the initial load',
        async () => {
            addMarker('42')
    ...
    it('measures a Turbo soft navigation from before-visit to load, '
        + 'preferring the X-Runtime header', async () => {
    ...
    it('falls back to the inline server ms when the response carries no X-Runtime header',
        async () => {
            const w = mount(LoadTimeBadge, { props })
