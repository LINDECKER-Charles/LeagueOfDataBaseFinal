...
import {
    ...
    switchSubmit,
    turboLoad,
} from '../../loader/loaderHarness'
import { BUILD_WARM_PATH, requestWarm } from '../../loader/warmBridge'

const mountLoader = () => mount(ResourceLoader, { props: loaderProps })
...
describe('ResourceLoader', () => {
    ...
    it('intercepts a cold resource visit, streams real progress + names, then performs the warm ' +
        'visit', async () => {
        const w = mountLoader()
    ...
    it('never surfaces the overlay on a warm destination (total 0), whatever the ' +
        'latency', async () => {
        const w = mountLoader()
    ...
    it('skips the gate when version/lang are unknown (falls through to a normal ' +
        'visit)', async () => {
        const w = mountLoader()
    ...
    it('names the four home resources and marks each ready as its group completes', async () => {
        ...
        es.emit('start', {
            total: 4,
            categories: { champion: 1, item: 1, runesReforged: 1, summoner: 1 },
        })
    ...
    it('rearms the inactivity watchdog on each event so a long but progressing warm ' +
        'is not cut', async () => {
        const w = mountLoader()
    ...
    it('gates a version/language switch from a batch-less page and covers the cold reload ' +
        'without streaming', async () => {
    ...
    it('gates a switch that lands on a list page: streams the batch then visits', async () => {
        ...
        await flushPromises() // prefs POST settles → gateSwitch opens the SSE
        expect(FakeEventSource.instances).toBe(1)
    ...
    it('warms a cold patch in place on request, streaming real progress, then resumes ' +
        'WITHOUT navigating', async () => {
    ...
    it('resumes a warm patch immediately without ever surfacing the overlay ' +
        '(total 0)', async () => {
        window.history.replaceState({}, '', '/builds/new')
        const w = mountLoader()
