...
// in addition to a cookie. The `framework.csrf_protection.check_header` config
// option needs to be enabled for the header to be checked
...
// When @hotwired/turbo handles form submissions, remove the CSRF cookie once a
...
export function generateCsrfToken (formElement) {
    ...
    if (csrfCookie && tokenCheck.test(csrfToken)) {
        const cookie = csrfCookie + '_' + csrfToken + '='
            + csrfCookie + '; path=/; samesite=strict';
...
export function removeCsrfToken (formElement) {
    ...
    if (tokenCheck.test(csrfField.value) && nameCheck.test(csrfCookie)) {
        const cookie = csrfCookie + '_' + csrfField.value
