...
final class BuildModerationController extends AbstractAdminController
{
    ...
    public function index(Request $request): Response
    {
        ...
        ['builds' => $builds, 'total' => $total] = $this->builds->searchPaginated(
            $query,
    ...
    public function unpublish(Build $build, Request $request): Response
    {
        ...
        $this->audit->log(
            AuditAction::AdminBuildHide,
            target: AuditTarget::of(AuditTarget::TYPE_BUILD, $build->getId(), $build->getName()),
        );
        $this->addFlash('success', sprintf(
            'Build « %s » dépublié — il reste visible par son auteur et via son lien de partage.',
            $build->getName()
        ));
