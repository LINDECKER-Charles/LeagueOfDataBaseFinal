...
final class UserModerationController extends AbstractAdminController
{
    ...
    public function index(Request $request): Response
    {
        ...
        ['rows' => $rows, 'total' => $total] = $this->users->searchPaginated(
            $query,
            $page,
            self::PER_PAGE,
        );
    ...
    public function ban(User $user, Request $request): Response
    {
        ...
        $this->audit->log(
            ...
            metadata: ['reason' => $reason === '' ? null : $reason],
        );
        $this->addFlash('success', sprintf(
            'Compte « %s » banni : connexion bloquée, profil et builds retirés du site public.',
            $user->getUsername(),
        ));
