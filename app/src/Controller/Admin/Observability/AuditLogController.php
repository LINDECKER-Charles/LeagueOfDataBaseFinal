...
final class AuditLogController extends AbstractAdminController
{
    ...
    public function index(Request $request): Response
    {
        ...
        ['rows' => $rows, 'hasMore' => $hasMore] = $this->query->recent(
            $this->filter($request),
            new PageWindow($page, self::PER_PAGE_LOGS),
        );
    ...
    {
        $page = $this->pageParam($request);
        ['rows' => $rows, 'hasMore' => $hasMore] = $this->query->forUser(
            $user,
            $this->filter($request),
            new PageWindow($page, self::PER_PAGE_LOGS),
        );
    ...
    {
        if ($error = $this->csrfError($request, 'admin_logs_purge', 'admin_logs')) {
            return $error;
        }
    ...
    private function purgeBounds(string $scope, Request $request): array
    {
        return match ($scope) {
            ...
            'before' => [
                $this->parseDate((string) $request->request->get('before', ''), endOfDay: false),
                false,
            ],
            default => [null, false],
        };
