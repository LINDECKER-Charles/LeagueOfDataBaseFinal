...
final class DonationController extends AbstractPageController
{
    ...
    /** Stripe amounts are minor units; the form talks euros. */
    ...
    public function index(): Response
    {
        return $this->render('donate/index.html.twig', [
            ...
            'minEuros' => intdiv(DonationTiers::MIN_CENTS, self::CENTS_PER_EURO),
            'maxEuros' => intdiv(DonationTiers::MAX_CENTS, self::CENTS_PER_EURO),
        ]);
    }
    ...
    {
        if ($rejected = $this->checkoutGate($request)) {
            return $rejected;
        }
    ...
    /**
     * Everything that must hold before a Stripe session is even attempted — a
     * valid token, a client under the rate limit and a configured gateway. Null
     * lets the checkout proceed.
     */
    private function checkoutGate(Request $request): ?RedirectResponse
    {
