 ...
 */
final class ApiKeyController extends AbstractPageController
{
    ...
    public function portal(Request $request): Response
    {
        ...
        return $this->render('api/portal.html.twig', [
            ...
            'usedThisMonth' => $key === null
                ? 0
                : $this->apiKeys->sumRequestsForMonth($key, new \DateTimeImmutable()),
            'dailyUsage' => $key === null
                ? []
                : $this->apiKeys->recentDailyUsage($key, self::USAGE_WINDOW_DAYS),
    ...
    {
        if (($rejected = $this->csrfRejection($request)) !== null) {
            return $rejected;
        }
        ...
        $this->traceKeyAction(AuditAction::ApiKeyCreate, $issued->key);
    ...
    public function regenerate(Request $request): RedirectResponse
    {
        ...
        $this->traceKeyAction(AuditAction::ApiKeyRegenerate, $issued->key);
    ...
    public function revoke(Request $request): RedirectResponse
    {
        ...
        $key->revoke();
        $this->entityManager->flush();
        $this->traceKeyAction(AuditAction::ApiKeyRevoke, $key);
    ...
    /** Null when the POSTed token is valid; otherwise the bounce back to the portal. */
    private function csrfRejection(Request $request): ?RedirectResponse
    {
        $token = (string) $request->request->get('_token');

        return $this->isCsrfTokenValid(self::CSRF_TOKEN_ID, $token)
            ? null
            : $this->backToPortalWithError('portal.flash.csrf');
    }

    /** Key lifecycle audit line — the public prefix only, never the secret. */
    private function traceKeyAction(AuditAction $action, ApiKey $key): void
    {
        $this->audit->log($action, target: AuditTarget::of(
            AuditTarget::TYPE_API_KEY,
            $key->getId(),
            $key->getKeyPrefix() . '…',
        ));
    }
