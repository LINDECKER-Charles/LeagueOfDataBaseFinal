 ...
 * Login is never blocked on verification — build creation is (see Build\BuildController).
 */
final class EmailVerificationController extends AbstractPageController
{
    ...
    public function resend(Request $request): Response
    {
        ...
        if (!$this->emailVerificationLimiter->create($user->getUserIdentifier())
            ->consume()
            ->isAccepted()
        ) {
    ...
    private function redirectAfterVerify(string $type, string $messageKey): Response
    {
        ...
        return $this->redirectToRoute(
            $this->getUser() instanceof User ? 'app_profile' : 'app_login'
        );
    }
    ...
    {
        return $this->backToOrigin($request, 'app_profile');
    }
}
