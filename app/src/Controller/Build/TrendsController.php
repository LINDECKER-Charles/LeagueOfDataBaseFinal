 ...
 * catalog).
 */
final class TrendsController extends AbstractPageController
{
    ...
    /**
     * The ?language= facet, kept only when it is a known Data Dragon locale
     * (else: no restriction).
     */
    private function requestedLanguage(Request $request): ?string
    {
        ...
        return $language !== null && $this->versionManager->languageExists($language)
            ? $language
            : null;
    }
    ...
    private function languageOptions(): array
    {
        ...
        $options = array_map(
            static fn (string $code): array =>
                ['value' => $code, 'label' => $labels[$code] ?? $code],
