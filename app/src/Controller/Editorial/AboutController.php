 ...
 */
final class AboutController extends AbstractPageController
{
    ...
    private function faqEntries(): array
    {
        return array_map(fn (string $id): array => [
            'id'       => $id,
            'question' => $this->translator
                ->trans(sprintf('faq.%s.question', $id), [], self::CATALOG),
            'answer'   => $this->translator
                ->trans(sprintf('faq.%s.answer', $id), [], self::CATALOG),
        ], self::FAQ_ENTRIES);
    }
}
