...
enum AuditAction: string
{
    ...
    /**
     * Coarse grouping used by the admin filter. An explicit table, not a
     * derivation from the dotted prefix: the arms are exhaustive and there is no
     * `default`, so — exactly like {@see label()} — a new case added without a
     * category fails loudly instead of being filed under Administration.
     */
    public function category(): AuditCategory
    {
        return match ($this) {
            ...
            self::BuildCreate, self::BuildUpdate,
            self::BuildDelete, self::BuildVote => AuditCategory::Build,
            self::ApiKeyCreate, self::ApiKeyRegenerate, self::ApiKeyRevoke => AuditCategory::ApiKey,
            self::AdminUserBan, self::AdminUserUnban, self::AdminUserDelete,
            self::AdminBuildHide, self::AdminBuildDelete,
            self::AdminApiClientRevoke, self::AdminApiClientCredit,
            self::AdminAnalyticsRollup, self::AdminLogsPurge => AuditCategory::Admin,
        };
    }
