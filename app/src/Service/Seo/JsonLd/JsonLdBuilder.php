...
final class JsonLdBuilder
{
     ...
     *
     * @param array{name:string, url?:?string, image?:?string, description?:?string} $identity
     * @return array<string,mixed>
     */
    public function person(array $identity): array
    {
    ...
    public function profilePage(array $profile): array
    {
        return [
            '@context'   => JsonLdEncoder::SCHEMA_CONTEXT,
            '@type'      => 'ProfilePage',
            'mainEntity' => $this->person($profile),
        ];
    }
    ...
    public function article(array $fields): array
    {
        $author = ($fields['authorName'] ?? '') !== ''
            ? $this->person([
                'name' => (string) $fields['authorName'],
                'url'  => $fields['authorUrl'] ?? null,
            ])
            : null;
        $about = ($fields['about'] ?? '') !== ''
