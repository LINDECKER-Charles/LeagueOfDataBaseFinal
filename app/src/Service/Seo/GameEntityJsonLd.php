...
final class GameEntityJsonLd
{
    ...
    public function runePath(array $rune, ?string $imageUrl, string $url): array
    {
        ...
        $entity = $this->entity('Thing', $url, 'rune-path', [
            ...
            'image' => $imageUrl,
        ], $this->scalarProperty('Slots', $slots === [] ? null : \count($slots)));
    ...
    {
        $properties = array_merge(
            $this->scalarProperty('Cooldown', $this->firstNumber($spell['cooldown'] ?? null), self::UNIT_SECONDS),
            $this->scalarProperty('Range', $this->firstNumber($spell['range'] ?? null), self::UNIT_UNITS),
     ...
     * @return array<string,mixed>
     */
    private function entity(string $type, string $url, string $fragment, array $fields, array $properties): array
    {
    ...
    private function mapped(mixed $source, array $labels, ?string $unit = null): array
    {
        ...
        foreach ($labels as $key => $label) {
            $properties = array_merge($properties, $this->scalarProperty($label, $source[$key] ?? null, $unit));
        }
