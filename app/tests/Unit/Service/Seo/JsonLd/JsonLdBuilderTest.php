...
final class JsonLdBuilderTest extends TestCase
{
    ...
    {
        $json = $this->builder->encode([
            'url' => 'https://example.com/champion/Aatrox',
            'name' => 'Ré?',
        ]);
    ...
    public function testBreadcrumbListPositionsAreOrdered(): void
    {
        ...
        self::assertSame(JsonLdEncoder::SCHEMA_CONTEXT, $graph['@context']);
        ...
        self::assertSame(
            'https://example.com/champion/Aatrox',
            $graph['itemListElement'][2]['item'],
        );
    }
    ...
    public function testItemListCapsEntriesAtMax(): void
    {
        $items = array_map(
            static fn (int $i): array => [
                'name' => "Item $i",
                'url' => "https://example.com/object/$i",
            ],
