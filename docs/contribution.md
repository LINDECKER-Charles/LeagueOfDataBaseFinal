...
- **Docker** + **Docker Compose** — la stack complète (PHP 8.5 / Symfony 7.4, micro-service Go, PostgreSQL) tourne en conteneurs. Rien à installer en local côté backend.
...
- **Docker** + **Docker Compose** — the full stack (PHP 8.5 / Symfony 7.4, Go micro-service, PostgreSQL) runs in containers. Nothing to install locally for the backend.
...
- **Docker** + **Docker Compose** — el stack completo (PHP 8.5 / Symfony 7.4, micro-servicio Go, PostgreSQL) corre en contenedores. Nada que instalar localmente para el backend.
